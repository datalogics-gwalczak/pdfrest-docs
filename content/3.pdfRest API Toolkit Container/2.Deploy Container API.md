...
- The `LICENSE_KEY` variable is required to properly license the server. Without setting this variable before deploying, pdfRest will run in a reduced capabilities mode. Sign up for a free account on [pdfRest.com](https://pdfrest.com/getstarted/) to request a key! Existing accounts can generate a key from the [Container section of the Account page](https://pdfrest.com/account).
